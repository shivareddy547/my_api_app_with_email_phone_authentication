...
`;
    ...
    // ===== Rails Model Selection State =====
    let selectedRailsModel = null;
    let selectedRailsRelationships = [];
            ...
            const endpointRef = {
                ...
                endpoint_metadata: {
                    ...
                    "x-controller-info": endpointInfo.extensions.controller_path ? {
                        ...
                        name: endpointInfo.extensions.controller
    ...
    const sanitizeReferenceComponents = (refComponents = {}) => {
        const clean = {};

        Object.entries(refComponents).forEach(([key, value]) => {
            if (!value) return;
            ...
            clean[key] = {
                ...
                // extract ONLY serializable endpoint data
                endpoint_data: value.endpoint_data
                    ? {
                        controller: value.endpoint_data.controller,
                        action: value.endpoint_data.action,
                        model: value.endpoint_data.model,
                        controller_path: value.endpoint_data.controller_path
                    }
                    : null
            };
        });
        ...
        return {
            ...
            endpoint_data: endpoint.endpoint_data
                ? {
                    ...
                    action: endpoint.endpoint_data.action,
                    model: endpoint.endpoint_data.model,
                    controller_path: endpoint.endpoint_data.controller_path
                }
                : null,

            metadata: endpoint.metadata ?? null,
            endpoint_metadata: endpoint.endpoint_metadata ?? null

            // ❌ DO NOT INCLUDE opBlock
        };
    };
    ...
    const sanitizeRequirements = (requirements) => {
        return requirements.map(req => ({
            ...
            text: req.text,
            active: req.active,
            isNewComponent: req.isNewComponent,
            ...
            componentDetails: req.componentDetails
                ? {
                    name: req.componentDetails.name,
                    domPath: req.componentDetails.domPath,
                    tagName: req.componentDetails.tagName,
                    textContent: req.componentDetails.textContent
                }
                : null,

            components: (req.components || []).map(c => sanitizeEndpoint(c)),

            reference_components: Object.fromEntries(
                Object.entries(req.reference_components || {}).map(([key, value]) => [
                    key,
                    sanitizeEndpoint(value)
                ])
            ),

            // NEW: Include Rails model info
            rails_model: req.rails_model ? {
                name: req.rails_model.name,
                table_name: req.rails_model.table_name,
                columns: req.rails_model.columns || {},
                relationships: req.rails_model.relationships || [],
                model_details: req.rails_model.model_details || {}
            } : null
        }));
    };
    ...
    const serializeSelections = (selections) => {
        return selections.map(sel => ({
            requirement: sel.requirement,
            text: sel.text,
            active: sel.active,
            integrate: sel.integrate,
            isNewComponent: sel.isNewComponent,
            componentType: sel.componentType,
            feature_types: sel.feature_types ?? [],
            component: sel.component,
            componentDetails: sel.componentDetails ? {
                name: sel.componentDetails.name,
                domPath: sel.componentDetails.domPath,
                textContent: sel.componentDetails.textContent,
                tagName: sel.componentDetails.tagName
            } : null,
            components: (sel.components || []).map(c => sanitizeEndpoint(c)),
            reference_components: Object.fromEntries(
                Object.entries(sel.reference_components || {}).map(
                    ([key, value]) => [key, sanitizeEndpoint(value)]
                )
            ),
            // ADD THESE FIELDS TO SAVE YAML RESPONSES
            yaml_response: sel.yaml_response || null,
            response_id: sel.response_id || null,
            response_timestamp: sel.response_timestamp || null,
            model_used: sel.model_used || null,
            provider_used: sel.provider_used || null,
            applied: sel.applied || false,
            applied_timestamp: sel.applied_timestamp || null,
            // NEW: Save Rails model info
            rails_model: sel.rails_model ? {
                name: sel.rails_model.name,
                table_name: sel.rails_model.table_name,
                columns: sel.rails_model.columns || {},
                relationships: sel.rails_model.relationships || [],
                model_details: sel.rails_model.model_details || {}
            } : null
        }));
    };
    ...
    const saveSelections = () => {
        const dataToSave = {
            requirements: serializeSelections(selections),
            globalFeatureRequest: globalFeatureRequest,
            globalFeatureDetails: globalFeatureDetails,
            timestamp: new Date().toISOString()
        };
        ...
        console.log("✅ Saved requirements:", dataToSave.requirements.length);
    };
    ...
    function loadSelections() {
        try {
            ...
            if (saved) {
                ...
                // Load all selections including YAML responses
                selections = Array.isArray(parsed.requirements) ? parsed.requirements : [];

                // Ensure YAML responses are properly loaded
                selections.forEach((req, index) => {
                    // Make sure the response data persists
                    if (req.yaml_response && !req.response_id) {
                        req.response_id = `req_${index}`;
                    }
                });

                globalFeatureRequest = parsed.globalFeatureRequest || globalFeatureRequest;
                globalFeatureDetails = parsed.globalFeatureDetails || globalFeatureDetails;

                console.log(`📂 Loaded ${selections.length} requirements with ${selections.filter(r => r.yaml_response).length} YAML responses`);
            }
        } catch (e) {
    ...
    const clearSelections = () => {
        // Clear everything including YAML responses
        selections = [];
        referenceComponents = {};
        globalFeatureRequest = "";
        globalFeatureDetails = "";
        selectedRailsModel = null;
        selectedRailsRelationships = [];
        ...
        updateCount();
        showNotification("🧹 All requirements and responses cleared", "info");
    };
    ...
    async function applyYamlChanges(yamlContent, requirementId) {
        ...
        showNotification("🚀 Applying changes to your project...", "info");
    ...
    async function fetchModels(provider) {
        ...
        showNotification(`📋 Fetching models for ${provider}...`, "info");
    ...
    async function fetchSwaggerEndpoints() {
        try {
            showNotification("📡 Fetching Swagger API endpoints...", "info");
    ...
    // ===== NEW: Rails Model Selection Functions =====
    async function fetchRailsModels() {
        try {
            showNotification("🔄 Fetching Rails models...", "info");

            const data = await fetchWithAuth('/api/get_rails_models', {
                method: "POST",
                body: JSON.stringify({})
            });

            console.log("✅ Rails models received:", data);

            if (data.models && Array.isArray(data.models)) {
                showNotification(`✅ Loaded ${data.models.length} Rails models`, "success");
                return data.models;
            } else {
                showNotification("❌ No Rails models found", "error");
                return [];
            }
        } catch (error) {
            console.error("❌ Error fetching Rails models:", error);
            showNotification(`❌ Failed to fetch models: ${error.message}`, "error");
            return [];
        }
    }

    // Function to fetch relationships for a specific model
    async function fetchModelRelationships(modelName) {
        try {
            showNotification(`🔄 Fetching relationships for ${modelName}...`, "info");

            const data = await fetchWithAuth('/api/get_model_relationships', {
                method: "POST",
                body: JSON.stringify({
            ...
            console.log(`✅ Relationships for ${modelName}:`, data);
            ...
            console.error(`❌ Error fetching relationships for ${modelName}:`, error);
            return [];
        }
    }

    // Function to show model selection modal
    function showModelSelectionModal(onModelSelect) {
        closeAllModals();
        ...
        modal.innerHTML = `
        <div style="display: flex; justify-content: space-between; align-items: center; margin-bottom: 20px; border-bottom: 1px solid #444; padding-bottom: 15px;">
            <h3 style="margin: 0; color: #00e0ff; font-size: 18px;">🏗️ Select Rails Model</h3>
            <button id="closeModelModal" style="
                background: none; 
                border: none; 
                color: #fff; 
                font-size: 24px; 
        ...
        <div style="margin-bottom: 15px;">
            <div style="font-size: 12px; color: #aaa; margin-bottom: 8px;">
                Select a Rails model for your endpoint. You can also include related models.
            </div>
            <div style="display: flex; align-items: center; gap: 10px;">
                <input type="text" id="modelSearch" placeholder="Search models..." style="
                    flex: 1;
                    padding: 8px;
                    border: 1px solid #555;
                    border-radius: 6px;
                    ...
                    font-size: 13px;
                ">
                <button id="refreshModelsBtn" style="
                    padding: 8px 12px;
                    background: #00e0ff;
                    border: none;
                    border-radius: 6px;
        ...
        <div id="modelsList" style="margin-bottom: 20px; max-height: 300px; overflow-y: auto;">
            <div style="text-align: center; padding: 40px; color: #666;">
                <div class="loader" style="
                    ...
                    margin: 0 auto 15px;
                "></div>
                <div>Loading Rails models...</div>
            </div>
        </div>

        <div id="selectedModelSection" style="display: none; margin-bottom: 20px;">
            <h4 style="color: #00ff88; margin-bottom: 10px; font-size: 16px;">✅ Selected Model: <span id="selectedModelName"></span></h4>
            
            <div id="relationshipsSection" style="display: none;">
                <h5 style="color: #00e0ff; margin-bottom: 8px; font-size: 14px;">🔗 Include Related Models</h5>
                <div id="relationshipsList" style="
                    background: rgba(255,255,255,0.05);
                    padding: 12px;
                    border-radius: 6px;
                    max-height: 200px;
        ...
        <div style="margin-top: 20px; text-align: right; border-top: 1px solid #444; padding-top: 15px;">
            <button id="cancelModelSelection" style="
                padding: 10px 20px;
                background: #666;
                border: none;
                border-radius: 6px;
                ...
                font-size: 13px;
            ">Cancel</button>
            <button id="confirmModelSelection" style="
                padding: 10px 20px;
                background: #00ff88;
                border: none;
                border-radius: 6px;
    ...
    `;
        ...
        function loadModels() {
            const modelsList = modal.querySelector("#modelsList");
        ...
        function renderModelsList(models) {
            const modelsList = modal.querySelector("#modelsList");
            const modelSearch = modal.querySelector("#modelSearch");

            if (models.length === 0) {
                modelsList.innerHTML = `
                    <div style="text-align: center; padding: 40px; color: #666;">
                        <div style="font-size: 48px; margin-bottom: 10px;">🏗️</div>
                        <div>No Rails models found</div>
            ...
            const searchTerm = modelSearch ? modelSearch.value.toLowerCase() : '';
            const filteredModels = models.filter(model =>
                !searchTerm ||
                model.name.toLowerCase().includes(searchTerm) ||
                (model.table_name && model.table_name.toLowerCase().includes(searchTerm))
            );
            ...
            filteredModels.forEach(model => {
                ...
                modelElement.style.cssText = `
                    ...
                    margin-bottom: 8px;
                    border-radius: 6px;
                    border-left: 4px solid ${model.name === selectedModel?.name ? '#00ff88' : '#555'};
                    cursor: pointer;
                    transition: all 0.2s;
                `;
                ...
                modelElement.innerHTML = `
                    <div style="display: flex; justify-content: space-between; align-items: center;">
                        <div>
                            <div style="font-weight: bold; color: #00e0ff; font-size: 14px; margin-bottom: 4px;">
                                ${model.name}
                            </div>
                            <div style="font-size: 11px; color: #ccc;">
                                Table: ${model.table_name || 'N/A'}
                            </div>
                            ${model.columns ? `
                            <div style="font-size: 10px; color: #888; margin-top: 4px;">
                                ${Object.keys(model.columns).length} columns
                            </div>
                            ` : ''}
                        </div>
        ...
        function updateSelectedModelDisplay() {
            const selectedModelSection = modal.querySelector("#selectedModelSection");
            const selectedModelName = modal.querySelector("#selectedModelName");
            const confirmBtn = modal.querySelector("#confirmModelSelection");

            if (selectedModel) {
                selectedModelSection.style.display = 'block';
                selectedModelName.textContent = selectedModel.name;
                confirmBtn.style.display = 'inline-block';
            } else {
                selectedModelSection.style.display = 'none';
                confirmBtn.style.display = 'none';
            }
        }

        async function loadRelationships(modelName) {
            const relationshipsSection = modal.querySelector("#relationshipsSection");
            const relationshipsList = modal.querySelector("#relationshipsList");
            ...
            const relationships = await fetchModelRelationships(modelName);
            ...
            relationships.forEach(relationship => {
                ...
                relationshipElement.style.cssText = `
                    display: flex;
                    align-items: center;
                    gap: 10px;
                    padding: 8px;
                ...
                const isSelected = selectedRelationships.has(relationship.name);

                relationshipElement.innerHTML = `
                    <input type="checkbox" id="rel_${relationship.name}" ${isSelected ? 'checked' : ''} style="margin: 0;">
                    <div style="flex: 1;">
                        <div style="display: flex; align-items: center; gap: 8px;">
                            ...
                            <span style="font-weight: bold; color: #00e0ff; font-size: 12px;">
                                ${relationship.name}
                            </span>
                        </div>
                        <div style="font-size: 10px; color: #ccc; margin-top: 2px;">
                            Model: ${relationship.class_name}
                            ${relationship.through ? `→ through: ${relationship.through}` : ''}
                        </div>
                    </div>
                `;

                const checkbox = relationshipElement.querySelector(`#rel_${relationship.name}`);
                checkbox.addEventListener('change', (e) => {
                    if (e.target.checked) {
                        selectedRelationships.add(relationship.name);
                    } else {
                        selectedRelationships.delete(relationship.name);
                    }
                });
        ...
        function getRelationshipColor(type) {
            const colors = {
                ...
                'has_many': '#00e0ff',
                'has_one': '#ffaa00',
                'has_and_belongs_to_many': '#ff66ff',
                'default': '#666'
            };
        ...
        // Event listeners
        modal.querySelector("#modelSearch")?.addEventListener("input", () => {
            renderModelsList(railsModels);
        });

        modal.querySelector("#refreshModelsBtn")?.addEventListener("click", () => {
            loadModels();
        });

        modal.querySelector("#closeModelModal")?.addEventListener("click", closeModal);
        modal.querySelector("#cancelModelSelection")?.addEventListener("click", closeModal);

        modal.querySelector("#confirmModelSelection")?.addEventListener("click", () => {
            if (selectedModel && onModelSelect) {
                onModelSelect({
                    model: selectedModel,
                    relationships: Array.from(selectedRelationships)
                });
            }
            closeModal();
        });
    ...
    // Helper function to update selected model display in the popup
    function updateSelectedModelDisplayInPopup(box, model, relationships = []) {
        const selectedModelInfo = box.querySelector("#selectedModelInfo");
        if (!selectedModelInfo) return;

        if (!model) {
            selectedModelInfo.innerHTML = `
                <div style="color: #666; text-align: center; font-size: 12px; padding: 15px;">
                    No Rails model selected
                </div>
            `;
            return;
        }
        ...
        if (relationships.length > 0) {
            relationshipsHTML = `
                <div style="margin-top: 10px; padding-top: 8px; border-top: 1px dashed #555;">
                    <div style="font-size: 11px; color: #00e0ff; margin-bottom: 4px;">Included Relationships:</div>
        ...
        selectedModelInfo.innerHTML = `
            <div style="display: flex; justify-content: space-between; align-items: flex-start;">
                <div style="flex: 1;">
                    <div style="font-weight: bold; color: #00ff88; font-size: 14px; margin-bottom: 4px;">
                        ${model.name}
                    </div>
                    <div style="font-size: 11px; color: #ccc; margin-bottom: 2px;">
                        Table: ${model.table_name || 'N/A'}
                    </div>
                    ${model.columns ? `
                ...
                <button id="clearModelSelection" style="
                    background: #ff4444;
                    color: #fff;
                    border: none;
                    padding: 4px 8px;
        ...
        const clearBtn = selectedModelInfo.querySelector("#clearModelSelection");
        if (clearBtn) {
            clearBtn.addEventListener("click", () => {
                box._selectedRailsModel = null;
                updateSelectedModelDisplayInPopup(box, null, []);
            });
        }
    }

    // ===== ENHANCED: Unified Popup with Integration Feature and Rails Model Selection =====
    ...
    <!-- NEW: Rails Model Selection Section -->
    <div id="railsModelSection" style="display: none; margin-bottom: 15px;">
        <div style="display: flex; justify-content: space-between; align-items: center; margin-bottom: 8px;">
            <div style="font-size: 13px; color: #00ff88;">🏗️ Rails Model:</div>
            <button id="selectRailsModel" style="
                background: #00ff88;
                color: #000;
                border: none;
                padding: 6px 12px;
                ...
                align-items: center;
                gap: 4px;
            ">🏗️ Select Model</button>
        </div>
        <div id="selectedModelInfo" style="min-height: 60px; border: 1px dashed #00ff88; border-radius: 8px; padding: 12px; background: rgba(0,255,136,0.05);">
            <div style="color: #666; text-align: center; font-size: 12px; padding: 15px;">
                No Rails model selected
            </div>
        </div>
...
`;
        ...
        const backendCheckbox = box.querySelector("#is_backend");
        const railsModelSection = box.querySelector("#railsModelSection");

        // Initialize Rails model section visibility
        if (backendCheckbox && railsModelSection) {
            backendCheckbox.addEventListener('change', (e) => {
                if (e.target.checked) {
                    railsModelSection.style.display = 'block';
                } else {
                    railsModelSection.style.display = 'none';
        ...
        // NEW: Add event listener for Rails model selection button
        const selectRailsModelBtn = box.querySelector("#selectRailsModel");
        if (selectRailsModelBtn) {
            selectRailsModelBtn.addEventListener("click", () => {
                showModelSelectionModal((modelData) => {
                    // Store selected model data
                    box._selectedRailsModel = modelData;
                    updateSelectedModelDisplayInPopup(box, modelData.model, modelData.relationships);
                });
            });
        }
        ...
        if (saveBtn) {
            saveBtn.addEventListener("click", () => {
                ...
                // NEW: Add Rails model data if selected
                let railsModelData = null;
                if (box._selectedRailsModel) {
                    railsModelData = {
                        name: box._selectedRailsModel.model.name,
                        table_name: box._selectedRailsModel.model.table_name || '',
                        columns: box._selectedRailsModel.model.columns || {},
                        relationships: box._selectedRailsModel.relationships || [],
                        model_details: {
                            attributes: Object.keys(box._selectedRailsModel.model.columns || {}),
                            associations: box._selectedRailsModel.relationships
                        }
                    };
                }
                ...
                const requirementData = {
                    ...
                    active: isActive,
                    // NEW: Add Rails model info
                    rails_model: railsModelData
                };
                ...
                showNotification(`✅ ${isNew ? 'New component' : 'Requirement'} saved for ${finalComponentName} with ${referenceComponentsArray.length} reference components${integrateChecked ? ` and ${apiEndpointsArray.length} API endpoints` : ''}${railsModelData ? ` and Rails model ${railsModelData.name}` : ''}`, "success");
            });
        }
    ...
    async function refreshModels() {
        ...
        showNotification(`🔄 Refreshing models for ${currentProvider}...`, "info");
    ...
    // ===== ENHANCED: Enhanced Review Modal with Integration Support and Rails Model Display =====
    function showReviewModal() {
        ...
        // Count Rails models
        const railsModelsCount = selections.filter(req => req.rails_model).length;
        ...
        modal.innerHTML = `
            ...
            <div style="margin-bottom: 20px;">
                <div style="font-size: 14px; color: #aaa; display: flex; gap: 15px; margin-bottom: 15px;">
                    ...
                    <span>🏗️ ${railsModelsCount} Rails model(s)</span>
                </div>
        ...
        const createRequirementElement = (req, index, isActive = true) => {
            ...
            // NEW: Rails Model HTML
            const railsModelHTML = req.rails_model ? `
                <div style="margin-top: 8px;">
                    <div style="font-size: 11px; color: #00ff88; font-weight: bold; margin-bottom: 4px;">🏗️ Rails Model:</div>
                    <div style="background: rgba(0,255,136,0.1); padding: 8px; border-radius: 4px; border-left: 3px solid #00ff88;">
                        <div style="font-weight: bold; color: #00ff88;">${req.rails_model.name}</div>
                        <div style="font-size: 10px; color: #ccc; margin-top: 2px;">
                            Table: ${req.rails_model.table_name || 'N/A'}
                            ${req.rails_model.relationships?.length > 0 ? `
                                <div style="margin-top: 4px;">
                                    <span style="color: #00e0ff;">Relationships:</span>
                                    ${req.rails_model.relationships.map(rel =>
                `<span style="margin-left: 4px; background: rgba(0,224,255,0.2); color: #00e0ff; padding: 1px 4px; border-radius: 3px; font-size: 9px;">${rel}</span>`
            ).join('')}
                                </div>
                            ` : ''}
                        </div>
                ...
                <div style="display: flex; justify-content: space-between; align-items: flex-start; margin-bottom: 8px;">
                    <div style="flex: 1;">
                        ...
                        <div style="display: flex; align-items: center; gap: 8px; margin-top: 4px;">
                            ...
                            ` : ''}
                            ${req.rails_model ? `
                                <span style="background: #00ff88; color: #000; padding: 2px 6px; border-radius: 10px; font-size: 10px; font-weight: bold;">🏗️ RAILS</span>
                            ` : ''}
                        </div>
                    </div>
        ...
        const addEventListenersToButtons = () => {
            ...
            // In the showReviewModal function, update the delete button handler:
            const deleteButtons = modal.querySelectorAll(".deleteReq");
            deleteButtons.forEach(btn => {
                btn.addEventListener('click', (e) => {
                    const index = parseInt(e.target.getAttribute("data-index"));
                    if (confirm("Delete this requirement and all associated data?")) {
                        // Remove the requirement including its YAML response
    ...
    // ===== ENHANCED: Show Edit Requirement Modal with Integration and Rails Model =====
    function showEditRequirementModal(requirement, index) {
        ...
        // Load Rails model data if exists
        if (requirement.rails_model) {
            box._selectedRailsModel = {
                model: requirement.rails_model,
                relationships: requirement.rails_model.relationships || []
            };
        }
        ...
        const isBackendChecked = requirement.feature_types?.includes('is_backend') || false;
        const hasRailsModel = requirement.rails_model ? true : false;
    ...
    `;
        ...
        <div style="margin-bottom: 15px;">
            ...
            <div style="display: grid; grid-template-columns: 1fr 1fr; gap: 8px;">
                ...
                <label style="display: flex; align-items: center; gap: 6px; cursor: pointer; padding: 8px; background: rgba(255,255,255,0.05); border-radius: 6px;">
                    <input type="checkbox" id="is_backend" class="feature-type-checkbox" style="margin: 0;" ${isBackendChecked ? 'checked' : ''}>
                    <span style="font-size: 13px;">⚙️ Backend</span>
                </label>
                <label style="display: flex; align-items: center; gap: 6px; cursor: pointer; padding: 8px; background: rgba(255,255,255,0.05); border-radius: 6px;">
        ...
        <!-- NEW: Rails Model Selection Section -->
        <div id="railsModelSection" style="display: ${isBackendChecked ? 'block' : 'none'}; margin-bottom: 15px;">
            <div style="display: flex; justify-content: space-between; align-items: center; margin-bottom: 8px;">
                <div style="font-size: 13px; color: #00ff88;">🏗️ Rails Model:</div>
                <button id="selectRailsModel" style="
                    background: #00ff88;
                    ...
                    align-items: center;
                    gap: 4px;
                ">🏗️ ${hasRailsModel ? 'Change Model' : 'Select Model'}</button>
            </div>
            <div id="selectedModelInfo" style="min-height: 60px; border: 1px dashed #00ff88; border-radius: 8px; padding: 12px; background: rgba(0,255,136,0.05);">
                ${hasRailsModel ? '' : `
                <div style="color: #666; text-align: center; font-size: 12px; padding: 15px;">
    ...
    `;
        ...
        // Update selected model display if exists
        if (hasRailsModel && requirement.rails_model) {
            updateSelectedModelDisplayInPopup(box, requirement.rails_model, requirement.rails_model.relationships || []);
        }
        ...
        if (saveBtn) {
            saveBtn.addEventListener("click", () => {
                ...
                showNotification(`✅ Requirement updated for ${finalComponentName} with ${referenceComponentsArray.length} reference components${integrateChecked ? ` and ${apiEndpointsArray.length} API endpoints` : ''}${railsModelData ? ` and Rails model ${railsModelData.name}` : ''}`, "success");
...
${escapeHtml(yamlContent)}
    ...
    async function sendSingleRequirement(requirement, index) {
        try {
            ...
            showNotification(`📤 Sending ${requirement.component} using ${selectedModel}...`, "info");

            const payload = {
                requirements: [sanitizeRequirement(requirement)], // Send only this requirement
                model: selectedModel,
                provider: selectedProvider,
                globalFeatureRequest,
                globalFeatureDetails,
                referenceComponents: Object.fromEntries(
                    Object.entries(referenceComponents || {}).map(([k, v]) => [
                        k,
                        sanitizeEndpoint(v)
                    ])
                )
            ...
            if (data.response || data.yaml_response) {
                ...
                // Save YAML response to the requirement
...
// Add helper function to sanitize single requirement
    function sanitizeRequirement(req) {
        return {
            requirement: req.requirement,
            text: req.text,
            active: req.active,
    ...
    async function sendToBackend() {
        ...
        try {
            // Show loader
            showLoader(`Sending ${activeRequirements.length} active requirements using ${selectedModel}...`);

            const payload = {
                requirements: sanitizeRequirements(activeRequirements),
                model: selectedModel,
                provider: selectedProvider,
                globalFeatureRequest,
                globalFeatureDetails,
            ...
            if (data.response || data.yaml_response) {
                ...
                // Save YAML response to each active requirement
                activeRequirements.forEach((req, index) => {
                    // Find the actual index in selections array
                    const actualIndex = selections.findIndex(selection =>
                        selection === req ||
                        (selection.component === req.component && selection.requirement === req.requirement)
                    );

                    if (actualIndex !== -1) {
                        selections[actualIndex].yaml_response = yamlResponse;
                        selections[actualIndex].response_timestamp = new Date().toISOString();
                        selections[actualIndex].response_id = responseId;
                        selections[actualIndex].model_used = selectedModel;
                        selections[actualIndex].provider_used = selectedProvider;
                    }
                });
